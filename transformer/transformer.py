...
class FeedForward:
    ...
    def forward(self, x):
        ...
        # Apply pre-norm which improves gradient flow in deep models instead of post-norm
        # that was used in the original paper

        x_normalised = self.layer_normalisation.forward(x)  # shape: (batch_size, max_sequence_length, d_model)
        output1 = self.dense1.forward(x_normalised)  # shape: (batch_size, max_sequence_length, d_ff)
        relu_out = np.maximum(0, output1)  # ReLU activation
        self.relu_mask = (output1 > 0)  # Cache mask for ReLU
        output2 = self.dense2.forward(relu_out)  # shape: (batch_size, max_sequence_length, d_model)
        add_output = self.add.forward([x, output2])  # shape: (batch_size, max_sequence_length, d_model)
        return add_output
    ...
    def backward(self, gradient_output, learning_rate):
        ...
        gradient_x_residual, gradient_output2 = self.add.backward(gradient_output, learning_rate)  # each of shape: (batch_size, max_sequence_length, d_model)
        ...
        gradient_relu = self.dense2.backward(gradient_output2, learning_rate)  # shape: (batch_size, max_sequence_length, d_ff)

        # Backprop through ReLU.
        # The derivative of ReLU is 1 for positive out1, 0 otherwise.
        gradient_output1 = gradient_relu * self.relu_mask  # shape: (batch_size, max_sequence_length, d_ff)

        # Backprop through Dense1.
        # dense1.backward returns gradient with respect to its input x_normalised.
        gradient_normalised = self.dense1.backward(gradient_output1, learning_rate)  # shape: (batch_size, max_sequence_length, d_model)

        # Backprop through Layer Normalisation
        gradient_input_layer_norm = self.layer_normalisation.backward(gradient_normalised, learning_rate)  # shape: (batch_size, max_sequence_length, d_model)

        gradient_input =  gradient_x_residual + gradient_input_layer_norm  # shape: (batch_size, max_sequence_length, d_model)
        return gradient_input
...
class GlobalSelfAttention(BaseAttention):
    ...
    def forward(self, x):
        ...
        x_normalised = self.layer_normalisation.forward(x)
        attention_output = self.mha.forward(query=x_normalised, key=x_normalised, value=x_normalised)
    ...
    def backward(self, gradient_output, learning_rate):
        ...
        gradient_x_residual, gradient_attention_output = self.add.backward(gradient_output, learning_rate)

        # As MultiHeadAttention receives 3 inputs (query, key, value), it returns 3 gradients during backpropagation
        gradient_query_normalised, gradient_key_normalised, gradient_value_normalised = self.mha.backward(gradient_attention_output, learning_rate)

        # Sum the gradients to get the gradient of the output of layer normalisation
        gradient_output_layer_norm = gradient_query_normalised + gradient_key_normalised + gradient_value_normalised

        # Gradient of the input of layer normalisation
        gradient_input_layer_norm = self.layer_normalisation.backward(gradient_output_layer_norm, learning_rate)
...
class CausalSelfAttention(BaseAttention):
    ...
    def forward(self, x):
        ...
        x_normalised = self.layer_normalisation.forward(x)
        attention_output = self.mha.forward(query=x_normalised, key=x_normalised, value=x_normalised, use_causal_mask=True)
        output = self.add.forward([x, attention_output])
        return output
...
class CrossAttention(BaseAttention):
    ...
    def forward(self, x, context):
        ...
        x_normalised = self.layer_normalisation.forward(x)
        context_normalised = self.layer_normalisation.forward(context)
        attention_output = self.mha.forward(query=x_normalised, key=context_normalised, value=context_normalised)
        output = self.add.forward([x, attention_output])
        return output
    ...
    def backward(self, gradient_output, learning_rate):
        ...
        gradient_context_normalised = gradient_key_normalised + gradient_value_normalised

        # Gradient of context
        gradient_context = self.layer_normalisation.backward(gradient_context_normalised, learning_rate)

        # Gradient of x that goes into layer normalisation
        gradient_x_input_layer_norm = self.layer_normalisation.backward(gradient_query_normalised, learning_rate)
...
if __name__ == '__main__':
    ...
    predictions = transformer.fit(input_data, target_data, epochs, batch_size, learning_rate=0.001, print_predictions=True)
    end_time = time.time()
    ...
    for i in range(num_sequences):
        ...
        for j in range(max_sequence_length):
            generated_token = np.argmax(predictions[i][j])
            generated_tokens.append(generated_token)
            if target_data[i, j] == generated_token:
                correct_generated_tokens += 1

        print(target_data[i], "  ", np.array(generated_tokens))
